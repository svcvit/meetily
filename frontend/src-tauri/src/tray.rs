...
/// - A local transcription model is ready (Sherpa, Parakeet, or Whisper)
async fn check_can_record<R: Runtime>(app: &AppHandle<R>) -> bool {
    ...
    // During onboarding, allow recording once any local transcription model is ready
    for (name, check) in [
        ("Sherpa", crate::sherpa_engine::commands::sherpa_has_available_models().await),
        ("Parakeet", crate::parakeet_engine::commands::parakeet_has_available_models().await),
        ("Whisper", crate::whisper_engine::commands::whisper_has_available_models().await),
    ] {
        match check {
            Ok(true) => return true,
            Ok(false) => {}
            Err(e) => {
                log::warn!("Tray: Failed to check {} models: {}", name, e);
            }
        }
    }
