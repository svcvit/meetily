...
export function SherpaModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        console.error('Failed to initialize Sherpa:', err);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      console.log('[SherpaModelManager] Setting up event listeners...');
      ...
      unlistenProgress = await listen<{ modelName: string; progress: number }>(
        ...
        (event) => {
          ...
          if (shouldUpdate) {
            console.log(`[SherpaModelManager] Progress update for ${modelName}: ${progress}%`);
    ...
    return () => {
      console.log('[SherpaModelManager] Cleaning up event listeners...');
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      await SherpaAPI.cancelDownload(modelName);
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      await SherpaAPI.deleteCorruptedModel(modelName);
  ...
  const recommendedModel = models.find(m =>
    m.name === 'sherpa-tdt-0.6b-v3-int8'
  );
